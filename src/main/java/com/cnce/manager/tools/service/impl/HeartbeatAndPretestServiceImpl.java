...
import com.cnce.manager.tools.dao.HeartbeatAndPretestDao;
...
@Service
public class HeartbeatAndPretestServiceImpl implements HeartbeatAndPretestService {

	private String jobName = JenkinsConstant.HB_7505_PRETEST;

	JenkinsUtil jenkins = new JenkinsUtil();

	@Autowired
	private HeartbeatAndPretestDao heartMapper;
	@Autowired
	...
	public List<HeartbeatAndPretestBlueDO> getAllAppInfo(Map<String, Object> map) {
		List<HeartbeatAndPretestBlueDO> list = heartMapper.getAllAppInfo(map);
		return list;
	}

	public int count(Map<String, Object> map) {
		return heartMapper.count(map);
	}

	public void startTest(String[] names, int num, String equipName, UserDO user) {
		...
		if (names.length > 0) {
			...
			String token = JenkinsJobInfo.JobName.JOB5.toString();
			// 开始使用时间
			...
			if(status == 1){
				...
				for(int i=0; i<3; i++){
					...
					manpower.setToolId(ToolsConstant.H_7505_ID);
					manpower.setToolName(ToolsConstant.H_7505_NAME);
				...
				powerMapper.insertList(manList);
			}
			// 测试结果入库
			LogsDO logDO = GenLogsDO.loadDO(user, ToolsConstant.H_7505_ID, status, ToolsConstant.H_7505_NAME,
					ToolsConstant.G_7505_LOG_PATH);
			logMapper.save(logDO);
			ReportDO reportDO = GenReportDO.loadDO(user, ToolsConstant.H_7505_ID, ToolsConstant.H_7505_NAME,
					ToolsConstant.G_7505_REPORT_PATH);
